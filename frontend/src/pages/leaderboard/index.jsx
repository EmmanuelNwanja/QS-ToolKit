...
export default function LeaderboardPage() {
  ...
  const CATEGORY_OPTIONS = [
    ...
    { value: 'professional', label: '🏗️ Professionals' },
            ...
            <div className="bg-gradient-to-r from-primary-800 to-primary-700 rounded-2xl p-5 text-white flex items-center justify-between flex-wrap gap-4">
              <div>
                ...
                <div className="flex items-center gap-3">
                  <span className="text-4xl font-bold font-display text-gold-400">
                    #{sort === 'rank_by_rating' ? myRank.rank_by_rating : myRank.rank_by_projects}
                  </span>
                  <div>
                    <p className="font-semibold">{myRank.public_id}</p>
                    <p className="text-primary-300 text-sm capitalize">{myRank.user_type}</p>
                  </div>
                </div>
              </div>
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={9} className="text-center py-10 text-gray-400">
                      No data yet.{category !== 'all' ? ` No ${category} entries in the leaderboard.` : ' Complete projects and collect client feedback to appear here!'}
                    </td>
                  </tr>
                ) : data.map((entry) => {
                  ...
                  return (
                    <tr key={entry.user_id} className={isMe ? 'bg-gold-50 font-medium' : ''}>
                      ...
                      <td>
                        <p className="font-semibold text-gray-900 font-mono text-sm">{entry.public_id}</p>
                        {isMe && <p className="text-xs text-primary-500">(You)</p>}
                      </td>
                      <td>
                        <span className="capitalize text-xs text-gray-500">{entry.user_type}</span>
                      </td>
