...
import { suggestConcrete } from './dimensionSuggestions';
...
export default function ConcreteForm({ onCalculate, loading }) {
  ...
  const conSuggest = useMemo(() => suggestConcrete({ elements }), [elements]);
          ...
          <div key={i} className="bg-gray-50 rounded-lg p-3 mb-3 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              {(el.type === 'column' || el.type === 'beam') && (
                <>
                  <div><label className="label text-xs">Depth (m)</label><input type="number" step="0.01" className="input py-1.5 text-xs" value={el.depth||''} onChange={e => updateElement(i,'depth',e.target.value)} />{conSuggest[i]?.suggestedDepth ? <span className="text-gold-600 text-xs ml-1">Suggestion: {conSuggest[i].suggestedDepth}m</span> : null}</div>
              ...
              {el.type === 'footing' && <div><label className="label text-xs">Depth (m)</label><input type="number" step="0.01" className="input py-1.5 text-xs" value={el.depth||''} onChange={e => updateElement(i,'depth',e.target.value)} />{conSuggest[i]?.suggestedDepth ? <span className="text-gold-600 text-xs ml-1">Suggestion: {conSuggest[i].suggestedDepth}m</span> : null}</div>}
              {conSuggest[i]?.volumeEstimate ? <p className="text-gold-600 text-xs col-span-2">Est. volume: {conSuggest[i].volumeEstimate}m³</p> : null}
